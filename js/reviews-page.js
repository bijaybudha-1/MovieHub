...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    s.style.cssText =
      "font-size:1.5rem;cursor:pointer;color:var(--text-muted);transition:0.15s;";
    s.dataset.value = i;
    s.addEventListener("mouseenter", () => {
      highlightStars(starsWrap, i);
    });
    s.addEventListener("click", () => {
      reviewRating = i;
      setStarsFixed(starsWrap, i);
    });
    starsWrap.appendChild(s);
  }
  starsWrap.addEventListener("mouseleave", () => {
    setStarsFixed(starsWrap, reviewRating);
  });
  starDiv.appendChild(starsWrap);

  function highlightStars(w, val) {
    w.querySelectorAll("i").forEach((s) => {
      const v = +s.dataset.value;
      s.className = v <= val ? "bx bxs-star" : "bx bx-star";
      s.style.color = v <= val ? "#f59e0b" : "var(--text-muted)";
    });
  }
  function setStarsFixed(w, val) {
    highlightStars(w, val);
  }
  ...
  if (data && data.results)
    document.getElementById("top-reviewed-scroll").innerHTML = data.results
      .slice(0, 15)
...
function renderUserReviews() {
  ...
  if (reviews.length === 0) {
    container.innerHTML =
      '<div class="empty-state"><i class="bx bx-edit"></i><h3>No reviews yet</h3><p>Write your first review above!</p></div>';
    return;
  }
