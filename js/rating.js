...
/* ── Get / Set Ratings ─────────────────────────────────────── */
function getRatings() { return safeParse(RATINGS_KEY); }
function setRatings(list) { safeWrite(RATINGS_KEY, list); }
...
function createStarRating(containerId, itemId, itemTitle, itemPoster, initialRating = 0) {
  ...
  wrapper.style.cssText = 'display:flex;align-items:center;gap:var(--space-md);';
  ...
  label.style.cssText = 'font-size:var(--fs-sm);color:var(--text-secondary);min-width:60px;';
...
function getRatingStats() {
  const ratings = getRatings();
  if (ratings.length === 0) return { total: 0, average: 0, distribution: new Array(10).fill(0) };
