...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("detail-bg").style.backgroundImage =
    `url(${getBackdropUrl(show.backdrop_path)})`;
  ...
  if (credits && credits.cast) {
    document.getElementById("cast-scroll").innerHTML = credits.cast
      .slice(0, 20)
      .map(
        (c) => `
  ...
  if (similar && similar.results) {
    document.getElementById("similar-scroll").innerHTML = similar.results
      .slice(0, 15)
