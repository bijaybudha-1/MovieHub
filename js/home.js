...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadTrending(timeWindow = 'day') {
        ...
        if (data && data.results) {
            container.innerHTML = data.results.slice(0, 15).map(m => buildMovieCard(m, 'movie')).join('');
            initScrollReveal();
        }
    }
    loadTrending('day');
    ...
    if (topRated && topRated.results) {
        document.getElementById('top-rated-grid').innerHTML =
            topRated.results.slice(0, 10).map(m => buildMovieCard(m, 'movie')).join('');
    }

    // ── Popular TV Shows ──
    const popularTV = await getPopularTVShows();
    if (popularTV && popularTV.results) {
        document.getElementById('popular-tv-grid').innerHTML =
            popularTV.results.slice(0, 10).map(m => buildMovieCard(m, 'tv')).join('');
    }

    // ── Featured Genres ──
    const genreIcons = {
        28: 'bx-bolt-circle', 12: 'bx-compass', 16: 'bx-palette', 35: 'bx-happy',
        80: 'bx-search-alt', 99: 'bx-camera-movie', 18: 'bx-mask', 10751: 'bx-group',
        14: 'bx-wand', 36: 'bx-book-open', 27: 'bx-ghost', 10402: 'bx-music',
        9648: 'bx-question-mark', 10749: 'bx-heart', 878: 'bx-planet', 53: 'bx-target-lock',
        10752: 'bx-shield', 37: 'bx-landscape'
    };
    const genreDescs = {
        28: 'Explosive thrills', 12: 'Epic journeys', 16: 'Animated worlds', 35: 'Laughs guaranteed',
        80: 'Criminal minds', 99: 'True stories', 18: 'Deep emotions', 10751: 'Fun for all',
        14: 'Magical realms', 36: 'Past stories', 27: 'Pure terror', 10402: 'Musical vibes',
        9648: 'Unsolved cases', 10749: 'Love stories', 878: 'Future worlds', 53: 'Edge of seat',
        10752: 'Battle epics', 37: 'Wild frontier'
    };
    const genres = await getMovieGenres();
    if (genres && genres.genres) {
        document.getElementById('genre-grid').innerHTML = genres.genres.slice(0, 12).map((g, i) => `
    ...
    if (people && people.results) {
        ...
        actorsScroll.innerHTML = people.results.slice(0, 20).map(p => `
        ...
        if (viewAllBtn) {
            viewAllBtn.addEventListener('click', () => {
                ...
                grid.innerHTML = people.results.map(p => `
    ...
    if (upcoming && upcoming.results) {
        document.getElementById('upcoming-grid').innerHTML = upcoming.results.slice(0, 6).map(m => {
