...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function displayUserDetails() {
    ...
    if (currentUser) {
      ...
      if (fullnameEl) fullnameEl.textContent = currentUser.fullname || 'User Fullname';
  ...
  function hideAllSections() { Object.values(sections).forEach(s => s && (s.style.display = 'none')); }
  function removeActiveClass() { navItems.forEach(i => i.classList.remove('active')); }
  function showSection(name) { hideAllSections(); if (sections[name]) sections[name].style.display = 'block'; }

  // init nav
  hideAllSections(); if (sections['Add to List']) sections['Add to List'].style.display = 'block'; if (navItems[0]) navItems[0].classList.add('active');
  navItems.forEach(item => item.addEventListener('click', function(){ removeActiveClass(); this.classList.add('active'); showSection(this.textContent.trim()); }));
  ...
  function showToast(message, type = 'success'){
    ...
    document.body.appendChild(t); setTimeout(()=>{ t.classList.add('hide'); setTimeout(()=>t.remove(),300); },3000);
  }
  ...
  function createListItem(m){
    ...
    const img = document.createElement('img'); img.src = m.poster_path || '../assets/images/movie-images.jpg'; img.alt = m.title||m.name||'Poster'; img.style.width='80px'; img.style.height='120px'; img.style.objectFit='cover';
    const content = document.createElement('div'); const title = document.createElement('div'); title.className='main-para'; title.textContent = m.title||m.name||'Untitled';
  ...
  function renderLists(){
    ...
    function renderInto(container, items, key){
      if(!container) return;
      container.innerHTML=''; container.classList.add('profile-list-grid');
      if(!items || items.length===0){ const empty=document.createElement('div'); empty.className='para'; empty.textContent='No items yet.'; container.appendChild(empty); return; }
      items.forEach(m=>{
        const { wrapper, favBtn, listBtn, removeBtn } = createListItem(m);
        favBtn.addEventListener('click', ()=>{ const favs = readList('moviehub_favorites'); if(!favs.find(x=>String(x.id)===String(m.id))){ favs.unshift(m); writeList('moviehub_favorites', favs); showToast(`Your "${m.title||m.name}" added a favorite`,'success'); } else showToast('Already in favorites','info'); renderLists(); });
        listBtn.addEventListener('click', ()=>{ const wls = readList('moviehub_watchlist'); if(!wls.find(x=>String(x.id)===String(m.id))){ wls.unshift(m); writeList('moviehub_watchlist', wls); showToast(`Added to watch list "${m.title||m.name}"`,'success'); } else showToast('Already in watchlist','info'); renderLists(); });
        removeBtn.addEventListener('click', ()=>{ let list = readList(key); list = list.filter(x=>String(x.id)!==String(m.id)); writeList(key,list); showToast(`"${m.title||m.name}" removed`,'success'); renderLists(); });
    ...
    if(ratingSection){
      ...
      if(!ratings || ratings.length===0){ const empty=document.createElement('div'); empty.className='para'; empty.textContent='No ratings yet.'; ratingContent.appendChild(empty); }
      else{
        ratings.forEach(r=>{
          const wrapper = document.createElement('div'); wrapper.className='profile-list-item d-flex gap-1 p-1 mt-1';
          ...
          const content = document.createElement('div'); const title = document.createElement('div'); title.className='main-para'; title.textContent = r.title||r.name||'Untitled';
          ...
          editBtn.addEventListener('click', ()=>{
            const val = prompt(`Update rating for "${r.title||r.name}" (1-10):`, r.rating||''); if(val===null) return; const n = Number(val); if(!n||n<1||n>10) return showToast('Please enter a number between 1 and 10','error');
            const ratingsList = readList('moviehub_ratings'); const idx = ratingsList.findIndex(x=>String(x.id)===String(r.id));
  ...
  renderLists(); window._renderProfileLists = renderLists;
});
